  ...
  ({ title, borders, className, type, ...rest }, ref) => {
    ...
    return (
      <div className="form-group relative flex flex-col-reverse">
        ...
        <label
          htmlFor={title}
          className="absolute left-[11px] text-[#707070] text-xs top-1 transform transition-all duration-300 ease-in peer-placeholder-shown:top-[13.5px] peer-placeholder-shown:text-base peer-focus:top-1 peer-focus:text-xs peer-focus:-translate-y-[0px] peer-focus:text-[#707070] peer:not(:placeholder-shown):top-1 peer:not(:placeholder-shown):text-xs peer:not(:placeholder-shown):-translate-y-[0px] peer:not(:placeholder-shown):text-primary"
        >
          {title}
        </label>
      </div>
